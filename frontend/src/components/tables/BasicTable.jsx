            ...
            <TableRow>
              {columnHeaders.map((header, index) => (
                <TableCell
                  key={index}
                  isHeader
                  className={`px-5 py-3 cursor-pointer px-5 py-3 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400 ${["action", "id"].includes(header.value.toLowerCase()) ? "text-center" : "text-start"}`}
                >
                  {header.value}
                </TableCell>
              ))}
