...
const ApplicantTypeForm = ({title=""}) => {
  ...
  const { isLoading, isError, data: result, error, isSuccess } = useQuery({ 
    queryKey: ["applicant-type", id], 
    queryFn: () => ApiClient.get(`applicant-types/${id}`).then((response) => response.data),
  ...
  useEffect(() => {
    ...
    if (isError && error?.response?.status === 404) {
      navigate("/applicant-type"); // Redirect to zoning list or home page
      toast.error("Applicant type not found!", { position: "bottom-right" });
    }
  ...
  const createMutation = useMutation({ 
    ...
    const updateMutation = useMutation({
      mutationFn: (data) => ApiClient.put(`applicant-types/${data?.id || 0}`, data).then((response) => response.data),
      onSuccess: (data) => {
        queryClient.invalidateQueries({ queryKey: ["applicant-type"] });
        if(data.success){
  ...
  return (
    <>
        ...
        description=""
    />
    <PageBreadcrumb pageTitle="Applicant Types" />
...
export default ApplicantTypeForm