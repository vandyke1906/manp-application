...
const ApplicantType = () => {
  ...
  const deleteMutation = useMutation({ 
    mutationFn: ({id}) => ApiClient.delete(`applicant-types/${id || 0}`).then((response) => response.data),
    onSuccess: (data) => {
      closeModal();
      queryClient.invalidateQueries({ queryKey: ["applicant-types"] });
      if(data.success){
    ...
    <div className="space-y-6">
        <ComponentCard title="">
              <Button size="sm" variant="primary" onClick={() => {
                navigate("/applicant-type/create");
              }}>
                New Applicant Type
              </Button>
              <GenericTable
