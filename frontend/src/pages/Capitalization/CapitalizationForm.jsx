...
const CapitalizationForm = ({title=""}) => {
  ...
  const { isLoading, isError, data: result, error, isSuccess } = useQuery({ 
    queryKey: ["capitalization", id], 
    queryFn: () => ApiClient.get(`capitalizations/${id}`).then((response) => response.data),
  ...
  useEffect(() => {
    ...
    if (isError && error?.response?.status === 404) {
      navigate("/capitalization"); // Redirect to capitalization list or home page
      toast.error("capitalization data not found!", { position: "bottom-right" });
    }
  ...
  const createMutation = useMutation({ 
    ...
    const updateMutation = useMutation({
      mutationFn: (data) => ApiClient.put(`capitalizations/${data?.id || 0}`, data).then((response) => response.data),
      onSuccess: (data) => {
        queryClient.invalidateQueries({ queryKey: ["capitalization"] });
        if(data.success){
  ...
  return (
    <>
        ...
        description=""
    />
    <PageBreadcrumb pageTitle="Capitalization" />
        ...
        <form ref={formRef} onSubmit={handleSave}>
          <div className="space-y-6">
                <div>
                    ...
                    <Input type="text" id="input" name="name" placeholder="Capitalization Name" defaultValue={obj?.name} />
                </div>
                <div>
