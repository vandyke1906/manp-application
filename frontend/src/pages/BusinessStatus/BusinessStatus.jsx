...
const BusinessStatus = () => {
  ...
  const deleteMutation = useMutation({ 
    mutationFn: ({id}) => ApiClient.delete(`business-statuses/${id || 0}`).then((response) => response.data),
    onError:(error) => console.log({error}),
    onSuccess: (data) => {
      closeModal();
      queryClient.invalidateQueries({ queryKey: ["business-statuses"] });
      if(data.success){
    ...
    <div className="space-y-6">
        <ComponentCard title="">
              ...
              <GenericTable columnHeaders={headers} tableData={result.data} 
                onEdit={(obj) => {
                  navigate(`/business-status/update/${obj.id}`);
                }} 
                onDelete={(obj) => {
