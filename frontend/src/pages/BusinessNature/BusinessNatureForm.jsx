...
const BusinessNatureForm = ({title=""}) => {
  ...
  const { isLoading, isError, data: result, error, isSuccess } = useQuery({ 
    queryKey: ["business-nature", id], 
    queryFn: () => ApiClient.get(`business-natures/${id}`).then((response) => response.data),
  ...
  useEffect(() => {
    ...
    if (isError && error?.response?.status === 404) {
      navigate("/business-nature"); // Redirect to business-nature list or home page
      toast.error("Nature of Business not found!", { position: "bottom-right" });
    }
  ...
  const createMutation = useMutation({ 
    ...
    const updateMutation = useMutation({
      mutationFn: (data) => ApiClient.put(`business-natures/${data?.id || 0}`, data).then((response) => response.data),
      onSuccess: (data) => {
        queryClient.invalidateQueries({ queryKey: ["business-nature"] });
        if(data.success){
        ...
        <form ref={formRef} onSubmit={handleSave}>
          <div className="space-y-6">
                <div>
                    ...
                    <Input type="text" id="input" name="name" placeholder="Nature of Business" defaultValue={obj?.name} />
                </div>
                <div>
